...
import { formatPeriod, getNextPayDate, parsePeriod, type PeriodUnit } from './periodMath';
...
export default function EditModalSub({ isOpen, onClose, subId }: EditModalSubProps) {
    ...
    useEffect(() => {
        ...
        async function getSub() {
            ...
            setStartDate(sub.dateStart)
            setNextPay(sub.dateEnd)
            const parsedPeriod = parsePeriod(sub.period)
            setPeriodCount(parsedPeriod.count)
            setPeriodUnit(parsedPeriod.unit)
            setSelectedValute(sub.currency)
            setUrlSub(sub.urlSub)
            setSelectedIcon({
                id: String(sub.id),
                name: sub.name,
                url: sub.logoUrl,
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center backdrop-blur-xs">
            ...
            <div className="z-10 w-260 bg-(--app-surface) border border-(--app-border) rounded-2xl p-6">
                <div className="flex items-center justify-between">
                    <h1 className="text-lg font-bold text-(--app-text)">Редактирование подписки</h1>
                    ...
                    <div className='flex flex-col gap-3'>
                        <div className='flex flex-row gap-3'>
                            ...
                            <div className='flex-1'>
                                ...
                                <input type="text" value={price} onChange={(e) => setPrice(Number(e.target.value))} placeholder={`${selectedValute === 'RUB' ? '0,00 ₽' : selectedValute === 'USD' ? '$ 0,00' : selectedValute === 'EUR' ? '€ 0,00' : '0,00'}`} className='focus:outline-0 w-full border border-(--app-border) rounded-lg p-2 px-3 mt-3' />
